...
def VAD(signal, sr, nFFT=512, win_length=0.025, hop_length=0.01, theshold=0.7):
    ...
    _stft = stft(signal, n_fft=nFFT, win_length=win_length_sample,
                 hop_length=hop_length_sample)
    ...
    for i in tqdm(range(nFrames)):
        ...
        oper[oper < 0] = 0
        smoothed_a_priori_SNR = smoothFactorDD * \
            previousGainedaPosSNR + (1-smoothFactorDD) * oper

        # V for MMSE estimate ([2](8))
        V = 0.1*smoothed_a_priori_SNR * \
            aPosterioriSNR_frame/(1+smoothed_a_priori_SNR)
        ...
        logGamma_frame = np.log(a10/a01) + gMeanLogLRT + \
...
if __name__ == '__main__':
    ...
    vad = VAD(signal, sr, nFFT=512, win_length=0.025,
